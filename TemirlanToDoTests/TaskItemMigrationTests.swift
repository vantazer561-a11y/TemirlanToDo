...
final class TaskItemMigrationTests: XCTestCase {
    ...
    //
    // Сериализуем случайную TaskItem в JSON-словарь, удаляем ключ `dueHasTime`, декодируем
    // обратно — задача должна декодироваться без ошибок, `dueHasTime` должен быть `false`,
    // остальные поля (включая `dueDate` без модификации) — без изменений.
    func testLegacyJSONWithoutDueHasTimeMigratesToFalse() {
    ...
    // Feature: task-time-and-notifications, Property 3: Инвариант nil-due
    // Validates: Requirements 1.4, 1.5, 1.8
    //
    // Часть 1: для случайной задачи с `dueDate == nil` и random `dueHasTime` после
    // encode → decode декодер форсирует `dueHasTime == false` (инвариант 1.4).
    // Часть 2: точечный JSON с `dueDate: null, dueHasTime: true` декодируется в
    // задачу с `dueHasTime == false`.
    func testDueDateNilImpliesNoTime() {
        // Часть 1: property over random nil-due tasks.
        PBT.forAll({
            TaskItem(
                id: UUID(),
            ...
            XCTAssertFalse(decoded.dueHasTime, "dueDate == nil ⇒ dueHasTime == false")
            XCTAssertEqual(decoded.id, task.id)
            XCTAssertEqual(decoded.title, task.title)
            XCTAssertEqual(decoded.notes, task.notes)
            XCTAssertEqual(decoded.isCompleted, task.isCompleted)
        ...
        // Часть 2: explicit JSON with dueDate=null, dueHasTime=true.
        let id = UUID()
        let createdAt = generateIntegerSecondDate()
        let updatedAt = generateIntegerSecondDate()
        // JSONEncoder использует .deferredToDate (timeIntervalSinceReferenceDate как Double).
        let json: [String: Any] = [
            "id": id.uuidString,
            "title": "stripe",
            "notes": "",
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(TaskItem.self, from: data)
            XCTAssertNil(decoded.dueDate)
            XCTAssertFalse(decoded.dueHasTime, "JSON dueDate=null + dueHasTime=true → dueHasTime=false")
            XCTAssertEqual(decoded.id, id)
        } catch {
            XCTFail("Failed to decode explicit JSON: \(error)")
        }
    }
    ...
    //
    // Для случайного `[TaskItem]` число вхождений ключа `"dueHasTime"` в JSON равно `tasks.count`.
    // Алфавит генератора (`generateRandomString`) не содержит символа `"`, поэтому подстрока
    // `"\"dueHasTime\""` (с обрамляющими кавычками) не может встретиться внутри полей `title`/`notes`.
